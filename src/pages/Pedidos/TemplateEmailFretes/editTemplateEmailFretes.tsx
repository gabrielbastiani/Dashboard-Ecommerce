...
const EditTemplateEmailFretes: React.FC = () => {
    ...
    async function refreshConfig() {
        try {
            ...
            const { data } = await apiClient.get(`/findUniqueEmailFreteOrder?slug_name_file_email=${slug_name_file_email}`);

            setName_file_email(data?.name_file_email);
            setSubject(data?.subject);
            setStatus(data?.status_frete);
    ...
    useEffect(() => {
        async function loadDatesTemplate() {
            try {
                const apiClient = setupAPIClient();
                const { data } = await apiClient.get(`/getUniqueTemplateEmailFreteStatus?slug_name_file_email=${slug_name_file_email}`);
    ...
    async function updateContentTemplate() {
        try {
            const apiClient = setupAPIClient();
            await apiClient.put(`/updateTemplateEmailOrderEmail?slug_name_file_email=${slug_name_file_email}`, {
                content: template
            });
    ...
    async function updatenametemplate() {
        try {
            ...
            if (name_file_email === "") {
                ...
                await apiClient.put(`/updateNameTemplateEmailFreteOrderStatus?slug_name_file_email=${slug_name_file_email}`,
                    {
    ...
    async function updateDateTemplate() {
        try {
            ...
            if (subject === "") {
                ...
                await apiClient.put(`/updateDateAllEmailFreteOrderStatus?templateOrderEmail_id=${idTemplate}`,
                    {
    ...
    async function updateStatus() {
        try {
            const apiClient = setupAPIClient();
            await apiClient.put(`/updateActiveStatusTemplateEmailFreteOrder?templateOrderEmail_id=${idTemplate}`);
...
export default EditTemplateEmailFretes;